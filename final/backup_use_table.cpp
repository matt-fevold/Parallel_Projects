...
int main(int argc, char **argv){
	...
	//these are a few hashes I found in the end of my file - should be able
	// to find a plaintext for these	
	//string hash_to_break = "53A5AFA1726AEE8660FB61C9420A2BBB";
	//string hash_to_break = "2673A577703A0FAACD61353A9F1215A3";
	//string hash_to_break = "D86A4A487B8FFD82118BC91C94177CD5";
	//
	...
	//this section is unnecessary for now... but I like looking at it.
	for(std::map<string, string>::const_iterator iter = global_rainbow_table.begin(); iter != global_rainbow_table.end(); ++iter)
	{
	//	this is how you loop through a map, here just for fun. should really remove...
...
string use_hash_chain( string password,string char_set, int len_min, int len_max, int index,int chain_length, int chain_number){
	...
	for(int i = 0; i < chain_length ; i++){
		...
		if(did_find_plaintext.compare("NO")!=0){
			...
			for(int j = 0;j<chain_length-1-i;j++){
		...
		// apply reduction (inverse step i) and hash. 
			...
			//it this way.
			plaintext_password = reduction_function(hashed_password,char_set,len_min,max_len,index,(chain_length -i-1));		
