...
//void *create_hash_chain(void *arg);
...
int main(int argc, char **argv){
	...
	//	pthread_mutex_lock(&lock);
	//	string final_hashed_password = "1234";	
...
void * thread_action(void *struct_data){
	...
	int thread_id = struct_data->thread_id;
	string character_set = struct_data->character_set;
	int plaintext_len_min = struct_data->len_min;
	int plaintext_len_max =  struct_data->len_max;
	int table_index = struct_data->table_index;
	int chain_len= struct_data->chain_length;
	int chain_num = struct_data->chain_number;

	int start = get_index(thread_id);
	int stop = get_index(thread_id+1);
...
string create_hash_chain( string password,string char_set, int len_min, int len_max, int index,int chain_length, int chain_number){
	...
	//int start=get_index(id);
	//int stop=get_index(id);
	//for(int i = start; i<stop;i++)
	//{
	//create password	
