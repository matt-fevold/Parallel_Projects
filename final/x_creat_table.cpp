...
//
char *create_hash_chain(char *password,char * character_set,int len_min, int len_max, int index,int chain_length, int chain_number);
char * hash_function(char *);
char *reduction_function(char *hashed_password,char *char_set,int len_min,int len_max,int index, int step_number);
void NTLM(char *key);
char *global_plaintext_password;
char *global_hashed_password;
...
int main(int argc, char **argv){
	...
	for(int i = 0; i < chain_num;i++){
		...
		//char test2 =  test;
		//cout<<"###: "<<test2<<endl;
		char *plaintext_password ="1234";  //(char ) ((100000+table_index)%(10*plaintext_len_max));
		//write first password into file
		myfile <<plaintext_password<< " , ";

		//create hash chains!
		char *final_hashed_password = create_hash_chain(plaintext_password,character_set, plaintext_len_min,plaintext_len_max,table_index,chain_len,chain_num);
...
char *create_hash_chain(char *password,char *char_set, int len_min, int len_max, int index,int chain_length, int chain_number){
...
char *reduction_function(char *hashed_password,char *char_set, int len_min,int len_max,int index, int step){
	
	//cout<<endl<<"::: "<<len_max<<endl;
	//because was getting seg faults since len_max was getting changed...
	int max_len = len_max;
	int current_step = step;
	char to_reduce[len_max];// = hashed_password; 
	for(int i = 0;i<33;i++){
	...
	return NULL;// ret_value;

}

char *hash_function(char *plaintext_password){
	//char * test = "asdf";
	char * test ; 
